  ...
  try {
    ...
    if (result.status === 'paid') {
      ...
      const ticketResult = await db.query(
        "SELECT * FROM tickets WHERE registration_id = $1 LIMIT 1",
        [result.registration.id]
      );
      const ticket = ticketResult.rows[0];
      const event = await Event.findById(result.registration.event_id);
      
      let qrCode = "";
      if (ticket) {
        const qrRes = await qrService.generateQR(ticket.id);
        qrCode = qrRes.qrCode;
      }
      ...
      return res.send(`
        ...
        <body>
          <div class="ticket-container">
            ...
            ${qrCode ? `<img src="${qrCode}" class="qr-img" alt="Ticket QR Code" />` : ""}
    ...
    // Redirect guest to payment checkout page
